...
def _get_user_data(user):
    ...
    and ensuring superusers and staff are recognized as admins.
    """
    role = UserProfile.Role.PLAYER  # Default fallback
    try:
        # The user.profile.is_admin property correctly checks for both the
        # 'admin' role and the user's 'is_staff' status.
        if user.profile.is_admin:
            role = UserProfile.Role.ADMIN
        else:
            role = user.profile.role
    except UserProfile.DoesNotExist:
        # For a user without a profile, check if they are a superuser
